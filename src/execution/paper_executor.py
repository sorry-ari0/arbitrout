...
class PaperExecutor:
    ...
    async def buy(self, asset_id: str, amount_usd: float, fallback_price: float = 0) -> ExecutionResult:
        ...
        if price <= 0:
            price = fallback_price  # Use leg entry_price when real feed unavailable
        if price <= 0:
    ...
    async def get_current_price(self, asset_id: str) -> float:
        price = await self.real.get_current_price(asset_id)
        if price <= 0:
            # Fall back to last known entry price from positions
            pos = self.positions.get(asset_id)
            if pos:
    ...
    def get_stats(self) -> dict:
        ...
        # Match sells to buys by asset_id to compute actual profit per round-trip
        wins = 0
        for sell in sells:
            aid = sell.get("asset_id", "")
            buy = next((b for b in self.trade_history if b["action"] == "buy" and b.get("asset_id") == aid), None)
            if buy and sell.get("price", 0) > buy.get("price", 0):
                wins += 1
