...
- **Recent changes (2026-03-28) — Maker-Only Enforcement + Multi-Outcome Arb Fix:**
  - **Maker-only orders enforced system-wide:** Paper executor `sell()` changed from taker fees to maker (0%) — matches real Polymarket executor which uses GTC orders. All manual exit paths (`force_close`, `manual_full_exit`, `manual_leg_exit`, `alert_approved`) now use `use_limit=True`. Position manager entry default changed to `_use_limit_orders=True`. Partial fill fallback uses `sell_limit()` instead of `sell()`. Exit engine AI-approved path always uses `use_limit=True`. Kill switch retains `use_limit=False` for emergencies.
  - **Multi-outcome arb exhaustiveness check:** Events with `total_yes < 0.70` (>30% unaccounted probability) are rejected as non-exhaustive. Prevents phantom spreads from events with unlisted outcomes (e.g., "None of the above"). Prior to fix, scanner detected 488 phantom opportunities and entered 3 losing trades from non-exhaustive events.
- **Previous changes (2026-03-28) — Debugging System + Synthetic Derivative Fixes:**
